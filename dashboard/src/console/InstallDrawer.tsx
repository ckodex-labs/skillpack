...
import { chamfer, Badge, CkButton, GradeChip, GalRail, TrustMark, DimMeter, SectionLabel } from './primitives';
import { PRIMITIVES, REGISTRIES, DIMS } from './lib/registry-data';
import { skrShare } from './lib/live';

// Persist the last-used share target so the operator doesn't retype it.
const shareTargetKey = 'skr-share-group';
// ============================================================
...
const InstallDrawer = ({ skill, onClose, onInstalled }) => {
  ...
  // Share-to-remote state.
  const [shareGroup, setShareGroup] = _useState2(() => (typeof window !== 'undefined' && window.localStorage.getItem(shareTargetKey)) || '');
  const [sharePhase, setSharePhase] = _useState2('idle'); // idle | busy | ok | err
  const [shareMsg, setShareMsg] = _useState2('');
  const runShare = async (dryRun) => {
    if (!shareGroup.trim() || !skill) return;
    try { window.localStorage.setItem(shareTargetKey, shareGroup.trim()); } catch (e) {}
    setSharePhase('busy'); setShareMsg(dryRun ? 'previewing…' : 'sharing…');
    const res = await skrShare(skill.name, shareGroup.trim(), { dryRun });
    if (res && res.ok) { setSharePhase('ok'); setShareMsg(res.message || 'shared'); }
    else { setSharePhase('err'); setShareMsg((res && res.error) || 'share failed'); }
  };
      ...
      <section role="dialog" aria-modal="true" aria-label={`Install ${skill.name}`}
        ...
        <div style={{ padding: '22px 24px 18px', borderBottom: '1px solid color-mix(in oklab, var(--ck-stroke) 22%, transparent)', position: 'sticky', top: 0, background: 'var(--ck-bg-0)', zIndex: 2 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
            ...
            {skill._live && (
              <a href={`/skill/${encodeURIComponent(skill.name)}`} title="Open full-page live assessment" className="ckr-focusring" style={{
                textDecoration: 'none', font: "700 10px 'JetBrains Mono', monospace", letterSpacing: '.06em',
                color: 'var(--ck-link)', padding: '6px 9px', clipPath: chamfer(4), whiteSpace: 'nowrap',
        ...
        <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div className="ckr-bento" style={{ background: 'var(--ck-bg-1)', clipPath: chamfer(10), boxShadow: 'inset 0 0 0 1.5px color-mix(in oklab, var(--ck-stroke) 38%, transparent)', padding: 16 }}>
            <SectionLabel style={{ marginBottom: 10 }}>Share to remote · git group</SectionLabel>
            <div className="ckr-input" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '0 12px', background: 'var(--ck-bg-2)', clipPath: chamfer(6), boxShadow: 'inset 0 0 0 1.5px color-mix(in oklab, var(--ck-stroke) 40%, transparent)', height: 40, marginBottom: 10 }}>
              <span className="ckr-glyph" aria-hidden="true" style={{ font: "500 13px 'JetBrains Mono', monospace", color: 'var(--ck-accent)' }}>⊳</span>
              <input value={shareGroup} onChange={(e) => setShareGroup(e.target.value)} placeholder="https://host/gitlab/group/skills-registry"
                aria-label="Remote group URL" style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', color: 'var(--ck-fg-1)', font: "400 12px 'JetBrains Mono', monospace", minWidth: 0 }} />
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <CkButton variant="primary" onClick={() => runShare(false)} disabled={!shareGroup.trim() || sharePhase === 'busy'}>
                {sharePhase === 'busy' ? '⊳ Sharing…' : `⊳ Share ${skill.name}`}
              </CkButton>
              <CkButton variant="ghost" onClick={() => runShare(true)} disabled={!shareGroup.trim() || sharePhase === 'busy'}>Dry-run</CkButton>
              <span style={{ marginLeft: 'auto', font: "500 9px 'JetBrains Mono', monospace", color: 'var(--ck-fg-mute)' }}>→ &lt;group&gt;/{skill.name} · no manual copy</span>
            </div>
            {sharePhase !== 'idle' && sharePhase !== 'busy' && (
              <div className="ckr-row" style={{ marginTop: 12, padding: '10px 12px', clipPath: chamfer(6), boxShadow: `inset 0 0 0 1.5px ${sharePhase === 'ok' ? 'var(--ck-accent)' : 'var(--ck-deny)'}`, background: sharePhase === 'ok' ? 'transparent' : 'color-mix(in oklab, var(--ck-deny) 8%, transparent)' }}>
                <span style={{ font: "500 11px 'JetBrains Mono', monospace", color: sharePhase === 'ok' ? 'var(--ck-fg-1)' : 'var(--ck-deny)', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                  {sharePhase === 'ok' ? '⊢ ' : '⊭ '}{shareMsg}
                </span>
              </div>
            )}
          </div>
