...
import { chamfer, Bento, Badge, CkButton, SectionLabel, GradeChip, DimMeter } from './primitives';
import { DIMS } from './lib/registry-data';
import { skrAssess } from './lib/live';
// ============================================================
// SkillPack Registry · Evolve page (re-assess a store skill)
// Pick a real canonical skill → re-run the LIVE 9-dimension
// assessment (skrAssess) → read the real grade / dims /
...
const Migration = ({ corpus, onSelect }) => {
  ...
  const run = async () => {
    if (!selected || typeof skrAssess !== 'function') return;
    setPhase('running'); setResult(null);
    const patch = await skrAssess(selected);
