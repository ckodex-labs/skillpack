...
const App = () => {
  ...
  const [theme, setTheme] = useState(() => {
    const t = localStorage.getItem('skr-theme');
    return VALID_THEMES.includes(t) ? t : 'ledger';
  });
  const [view, setView] = useState(() => localStorage.getItem('skr-view') || 'overview');

  // Boot flow: splash → auth → console.
  const [phase, setPhase] = useState('boot'); // boot | auth | app
  const [session, setSession] = useState(() => {
    const t = localStorage.getItem('skr-token');
  ...
  const [layout, setLayout] = useState(() => localStorage.getItem('skr-layout') || 'list');
  const [selected, setSelected] = useState(null);
  const [installed, setInstalled] = useState(() => new Set(SKILLS.filter(s => s.installed).map(s => s.id)));

  useEffect(() => { document.documentElement.setAttribute('data-theme', theme); localStorage.setItem('skr-theme', theme); }, [theme]);
  useEffect(() => { localStorage.setItem('skr-view', view); }, [view]);
  useEffect(() => { localStorage.setItem('skr-layout', layout); }, [layout]);
  ...
  useEffect(() => {
    ...
    (async () => {
      let healthy = false;
      if (window.skrHealth) {
        const h = await window.skrHealth();
        healthy = !!h.healthy;
        if (alive) setHealth({ ...h, probed: true });
      }
      if (!window.skrLoadCanonical) return;
      let res;
      try {
        res = await window.skrLoadCanonical();
      } catch (_e) {
      ...
      if (window.skrAssessAll) {
        setAssessProgress({ done: 0, total: res.skills.length });
        window.skrAssessAll(res.skills, {
          concurrency: 8,
          stopped: () => !alive,
  ...
  const enterConsole = (token) => {
    if (token) {
      ...
      localStorage.setItem('skr-token', token);
  ...
  const signOut = () => {
    ...
    localStorage.removeItem('skr-token');
    setSession(null);
    setView('overview');
    setPhase('auth');
  };
  ...
  const handleSelect = (skill) => {
    ...
    if (!health.healthy || !window.skrAssess) return;
    setLiveSkills(prev => (prev || []).map(s => s.id === skill.id ? { ...s, _assessing: true } : s));
    window.skrAssess(skill).then(patch => {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--ck-bg-0)', color: 'var(--ck-fg-1)' }}>
      ...
      <main id="main" data-screen-label={`web dashboard · ${view}`} style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        {view === 'discovery' && <Discovery health={health} apiBase={window.SKR_API} registries={REGISTRIES} />}
