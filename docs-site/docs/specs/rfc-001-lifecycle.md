...
### 3.4 `intent_to_invoke(skill, message)` — Formal Definition

`intent_to_invoke` is a Boolean predicate that guards the L2 → L3
promotion transition (§3.2). It signals that the current user message
contains **direct, specific evidence** that the user wants to invoke
this skill in the current turn — beyond the scoring threshold alone.

**Formal signature:**

```
intent_to_invoke(s: Skill, msg: String) → Bool
```

**Evaluation rules (ordered; first match wins):**

| Priority | Rule                                                                   | Result                                                        |
| -------- | ---------------------------------------------------------------------- | ------------------------------------------------------------- |
| 1        | `s.runtime.implicitInvocation == false ∧ ¬explicit_mention(s, msg)`    | `false` — non-implicit skill requires explicit mention        |
| 2        | `explicit_mention(s, msg)`                                             | `true` — user named or @-tagged the skill                     |
| 3        | `keyword_match(s.ontology.triggers, msg) ∧ score ≥ τ_full_load`        | `true` — ontology trigger keyword present and score qualifies |
| 4        | `keyword_match(s.ontology.subjects, msg) ∧ score ≥ τ_full_load + 0.10` | `true` — subject keyword with elevated threshold              |
| 5        | `score ≥ τ_full_load ∧ s.runtime.implicitInvocation == true`           | `true` — implicit skill, score alone sufficient               |
| 6        | *(default)*                                                            | `false`                                                       |

**Sub-predicates:**

```
explicit_mention(s, msg):
  → true  iff  any of s.name, s.metadata.name, s.ontology.triggers[]
               appears verbatim (case-insensitive) in msg

keyword_match(terms[], msg):
  → true  iff  any term in terms[] appears in msg
               (case-insensitive; partial word match at word boundary)
```

**Invariants:**

- `intent_to_invoke` MUST NOT be called before `score ≥ τ_full_load` is
  established; the transition table entry `score ≥ τ_full_load ∧
  intent_to_invoke` is a conjunction.
- `intent_to_invoke` is stateless per turn; it does not depend on previous
  invocations or session history.
- For skills without an `ontology` block: rules 3 and 4 reduce to `false`;
  rules 2 and 5 still apply (backward-compatible with v1 manifests).
- `explicit_mention` is a sufficient condition independent of score; a user
  explicitly requesting a skill bypasses the score gate and sets
  `intent_to_invoke = true` even if `score < τ_full_load`, as long as
  `S_pol = 1` (policy pre-filter passes).

**Implementation note:** harnesses without embedding infrastructure
SHOULD implement rules 1–4 only (lexical path). The result is
conservative (never over-promotes) and backward-compatible.
