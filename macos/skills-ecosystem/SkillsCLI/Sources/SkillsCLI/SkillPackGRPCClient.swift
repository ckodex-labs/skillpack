...
public final class SkillPackGRPCClient {
    ...
    // MARK: - T-07 STX Lifecycle projections (gRPC surface)

    /// Publish a lifecycle decision event via gRPC (mirrors STXPublisher.publishLifecycleEvent).
    /// Maps LifecycleDecisionBundle → Skillpack_V1_LifecycleEventRequest when the proto is available.
    /// Until proto generation is wired, falls back to the promote/demote calls below.
    public func promoteViaSTX(skillName: String) throws -> (success: Bool, message: String) {
        var request = Skillpack_V1_PromoteSkillRequest()
        request.candidateName = skillName
        let response = try client.promoteSkill(request).response.wait()
        return (response.success, response.message)
    }

    public func demoteViaSTX(skillName: String) throws -> (success: Bool, message: String, skillName: String) {
        var request = Skillpack_V1_PromoteSkillRequest()
        request.candidateName = skillName
        let response = try client.promoteSkill(request).response.wait()
        return (response.success, response.message, response.skillName)
