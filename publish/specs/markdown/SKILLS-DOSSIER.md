...
## §7. CLIENT-SPEC v0.1.0 Compliance (Cross-Cutting) **[C]**

> Unified client transport contract, canonical data model, and error taxonomy compliance audit. All changes driven by `schemas/skills-specs-next/CLIENT-SPEC.md` (v0.1.0, 2026-05-28).
> Status: **all four audited clients pass compilation + unit tests**.

### 7.1 Transport Contract — §3.1 (Discovery, Health, Auth)

| Client                | `SKILLPACK_API_URL` discovery                                                                                                                    | `SKILLPACK_TOKEN` Bearer auth                                                                | Health probe (2s timeout)                                                   | Test file                                                                                      |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| **CLI (Rust)**        | `CanonicalStoreClient::from_env()` reads env var; defaults to `http://localhost:50051`                                                           | `add_auth()` injects `authorization: Bearer <token>` metadata on every tonic `Request`       | `connect_timeout(2s)` on tonic `Channel`                                    | `crates/skillpack-adapters/src/grpc_client.rs` (lines 44–50, 52–57, 59–66)                     |
| **VS Code Extension** | `SkillPackApiClient` constructor reads `process.env.SKILLPACK_API_URL`, falls back to `vscode.workspace.getConfiguration("skillpack").serverUrl` | `getToken()` checks env var first, then VS Code `SecretStorage` (`skillpack.token`) per §6.1 | `healthProbe()` uses `AbortController` with 2s timeout                      | `vscode-extension/src/utils/api.ts` (lines 48–54, 55–65, 70–86)                                |
| **Web Dashboard**     | Reads `window.__SKILLPACK_API_URL__` at module load (injected at build time)                                                                     | `fetchWithAuth()` injects `Authorization: Bearer <token>` header                             | `healthProbe()` uses `AbortController` with 2s timeout                      | `dashboard/src/lib/client.ts` (lines 8–12, 15–27, 29–43)                                       |
| **macOS (Swift)**     | `STXPublisher.init()` reads `SKILLPACK_API_URL` from `ProcessInfo.processInfo.environment`; defaults to `http://localhost:50051`                 | `applyAuth(to:)` injects `Authorization: Bearer <token>` header                              | `checkHealth()` uses `URLRequest.timeoutInterval = 2.0` + semaphore wait 2s | `macos/skills-ecosystem/SkillsCore/Sources/SkillsCore/STXPublisher.swift` (lines 17–38, 42–59) |

**Audit notes:**
- macOS previously used `SKILLS_STX_ENDPOINT` / `SKILLS_STX_TOKEN` — renamed to `SKILLPACK_API_URL` / `SKILLPACK_TOKEN` and default endpoint changed from `http://localhost:8080` to `http://localhost:50051`.
- macOS `checkHealth()` timeout was 3.0s — corrected to 2.0s.
- VS Code `SecretStorage` fallback is new; previously only env var was supported.

### 7.2 Canonical Data Model — §2 (Generated Types)

All clients now consume types generated from `client-model.schema.json` (single source of truth) via the generation pipeline:

| Language   | Generator                                                   | Output location                                                                             | CI gate                  |
| ---------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------- | ------------------------ |
| TypeScript | `json-schema-to-typescript` with `--unreachableDefinitions` | `dashboard/src/generated/client-model.ts`, `vscode-extension/src/generated/client-model.ts` | `make check-schema-sync` |
| Rust       | `schemars` + `serde_derive`                                 | `crates/skillpack-adapters/src/generated/client_model.rs`                                   | `cargo check`            |
| Swift      | Custom generator / hand-maintained                          | `macos/skills-ecosystem/SkillsCore/Sources/SkillsCore/Generated/ClientModel.swift`          | `swift build`            |

**Type drift fixed:**
- `dashboard/src/components/SkillScorecard.tsx` — removed local `DimensionScore` interface (fields `id`, `name`); now imports canonical `DimensionScore` with `dimensionId` / `dimensionName` from `../lib/client` (re-exported from generated `client-model.ts`).
- `dashboard/src/components/index.ts` — updated `DimensionScore` export to canonical source.
- `vscode-extension/src/utils/html.ts` — `SkillSearchResult` extends canonical `SkillSummary`; `getSkillDetailsHtml()` uses canonical `grade`, `displayName`, `name` fields.
- `vscode-extension/src/commands/registry.ts` — uses canonical `SkillSummary` type; tree provider labels use `displayName` / `tier` / `id`.
- `vscode-extension/src/providers/registryTree.ts` — uses canonical `SkillSummary` type.

### 7.3 Error Taxonomy — §5 (Structured Errors + Per-Client Rendering)

| Client            | `ClientError` type usage                                               | Rendering function                                                                                                                                               | Test coverage                                                                               |
| ----------------- | ---------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| **CLI**           | `crate::generated::client_model::ClientError` imported in `cli/mod.rs` | `render_client_error_cli()` — maps `Severity` → colored prefix (`ERROR` red, `WARN` yellow, `INFO` dimmed) + `clientHint.cli`                                    | `grpc_client.rs` tests verify error enum deserialization                                    |
| **VS Code**       | `import type { ClientError } from "../generated/client-model"`         | `renderClientErrorVscode()` — maps severity → `showErrorMessage` / `showWarningMessage` / `showInformationMessage`; renders `clientHint.vscode` as detail button | `vscode-extension/src/utils/api.test.ts` — "should render structured errors correctly"      |
| **Web Dashboard** | `import type { ClientError } from "../../generated/client-model"`      | `renderClientErrorWeb()` — maps severity → banner color + dismiss action; renders `clientHint.web`                                                               | `dashboard/src/lib/client.test.ts` — "should render structured errors with severity levels" |
| **macOS**         | `SkillsError.clientError(ClientError)` case in `SkillsError.swift`     | `errorDescription` returns `clientError.message`; `macosHint` returns `clientHint.macos`                                                                         | `SkillsCoreTests.swift` — transport contract tests include error type imports               |

### 7.4 Accessibility — §8 (CLI Flags)

| Flag         | Implementation                                                                                                                                  | Test                                                     |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
| `--no-color` | `colored::control::set_override(false)` when `cli.no_color == true`                                                                             | Implicit via compilation                                 |
| `--quiet`    | New `static QUIET: AtomicBool` + `cli_println!` macro; suppresses non-error stdout when `--quiet` is set; `eprintln!` left untouched for errors | `quiet_flag_suppresses_cli_println` test in `cli/mod.rs` |

**Scope:** 144 `println!` calls in `cli/mod.rs` replaced with `cli_println!` macro. `eprintln!` calls (errors) remain unconditional.

### 7.5 Test Coverage Matrix

| Client                | Framework                    | Tests          | Key test categories                                                                                                                            |
| --------------------- | ---------------------------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| **VS Code Extension** | Vitest                       | **10 passed**  | Env discovery, token injection (present/absent), health probe, structured error rendering, canonical enum values                               |
| **Web Dashboard**     | Vitest                       | **6 passed**   | API URL discovery, Bearer token injection, health probe, structured error rendering (fatal/error/warning/info)                                 |
| **CLI (Rust)**        | `cargo test` + `serial_test` | **101 passed** | Env discovery (`SKILLPACK_API_URL`), token injection (`SKILLPACK_TOKEN`), auth header presence/absence, error type deserialization, quiet flag |
| **macOS (Swift)**     | Swift Testing                | **6 passed**   | Endpoint discovery (`SKILLPACK_API_URL`), token presence (`SKILLPACK_TOKEN`), auth header injection, default endpoint fallback                 |

**Compilation gates:** `tsc --noEmit` clean (dashboard + vscode-extension), `cargo check` clean, `swift build` clean, `make check-schema-sync` clean.

### 7.6 Files Changed

```
vscode-extension/src/utils/api.ts              # token discovery, SecretStorage fallback, health probe
vscode-extension/src/utils/api.test.ts       # +1 health probe test (10 total)
vscode-extension/src/utils/state.ts          # +extensionContext for SecretStorage access
vscode-extension/src/extension.ts             # store context in state on activation
vscode-extension/src/utils/html.ts           # canonical SkillSummary usage
vscode-extension/src/commands/registry.ts     # canonical SkillSummary, API method fixes
vscode-extension/src/providers/registryTree.ts # canonical SkillSummary
dashboard/src/lib/client.ts                 # canonical type re-exports
dashboard/src/lib/client.test.ts            # 6 tests (env, auth, health, errors)
dashboard/src/components/SkillScorecard.tsx  # canonical DimensionScore (removed local drift)
dashboard/src/components/index.ts            # canonical DimensionScore export
dashboard/src/components/RadarChart.tsx      # canonical DimensionScore import
dashboard/src/app/page.tsx                  # pass canonical dimensions directly
crates/skillpack-adapters/src/grpc_client.rs # env discovery, auth, 2s connect timeout, tests
crates/skillpack-adapters/src/cli/mod.rs     # --quiet flag, cli_println! macro, 144 replacements
crates/skillpack-adapters/Cargo.toml         # +serial_test dev dependency
macos/skills-ecosystem/SkillsCore/Sources/SkillsCore/STXPublisher.swift
                                              # env var rename, default endpoint, 2s timeout, testable init
macos/skills-ecosystem/SkillsCore/Tests/SkillsCoreTests/SkillsCoreTests.swift
                                              # 6 transport contract tests
```

### 7.7 Remaining Gaps

| Gap                                                                       | CLIENT-SPEC section | Class   | Reason                                                                                              |
| ------------------------------------------------------------------------- | ------------------- | ------- | --------------------------------------------------------------------------------------------------- |
| ~~gRPC streaming (`AssessStream`, `SyncAgentsStream`)~~                   | §3.2                | **[C]** | Server streams real dimension progress + assessment complete events                                 |
| ~~Cache invalidation (5-min skill list, 2-min detail, 1-min assessment)~~ | §4.1                | **[C]** | `TtlCache` wired; `QueryService` background task invalidates on `SkillUpdated`/`SyncCompleted`      |
| ~~Optimistic updates + rollback~~                                         | §4.2                | **[C]** | REST `POST/PATCH/DELETE /skills` mutate shared cache + publish events → SSE push + cache invalidate |
| ~~Event delivery (gRPC streaming / SSE / polling)~~                       | §4.3                | **[C]** | `EventBus` + gRPC streaming + SSE `/events` + `/health` with `protocolVersion`                      |
| ~~Capability negotiation (protocol version handshake)~~                   | §3.1 step 3         | **[C]** | REST clients read `protocolVersion` from `/health` JSON                                             |
| ~~macOS Keychain token storage~~                                          | §6.1                | **[C]** | `KeychainTokenStore.save/load/delete` wraps Security framework; env var > Keychain priority         |
| ~~Bearer token auth~~                                                     | §6                  | **[C]** | `SKILLPACK_API_TOKEN` env var enables auth on HTTP (axum middleware) + gRPC (tonic interceptor)     |
| ~~Production packaging~~                                                  | —                   | **[C]** | Multi-stage `Dockerfile` + `docker-compose.yml` with file-backed DuckDB volume + healthcheck        |
| ~~Graceful shutdown~~                                                     | —                   | **[C]** | `tokio::signal::ctrl_c()` triggers clean exit from gRPC + HTTP servers                              |
| ~~File-backed persistence~~                                               | —                   | **[C]** | `SKILLPACK_DB_PATH` env var selects `DuckDbRepository::open(path)` vs `in_memory()`                 |
| gRPC health check (`grpc.health.v1`)                                      | §3.1                | `[P]`   | Blocked on `tonic-health` compatibility with tonic 0.14; custom `GetVersion` RPC pending            |
| VS Code `skillpack.serverUrl` config still accepted                       | §3.1                | `[C]`   | Accepted as fallback after env var; spec says env var primary, but does not forbid config fallback  |

### 7.8 Capability Negotiation — §3.1 step 3 (Completed 2026-05-30)

> Per CLIENT-SPEC.md §3.1: "Capability negotiation: Server returns supported protocol version; client selects highest mutual version."
> Also §2.3: "Clients MUST reject responses with a schema version greater than their supported maximum."

#### Implementation

All REST clients now read `protocolVersion` (integer) from the JSON body of `GET /health` and compare against a client-side `SUPPORTED_PROTOCOL_VERSION = 1`.

| Client          | Health probe return type                 | Version parsing                          | Rejection behavior                                                                 | Test coverage                        |
| --------------- | ---------------------------------------- | ---------------------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------ |
| **Dashboard**   | `Promise<HealthResult>`                  | `resp.json().protocolVersion`            | `assess()` throws with version mismatch message                                    | 3 new Vitest tests                   |
| **VS Code**     | `Promise<{ healthy; protocolVersion? }>` | `resp.json().protocolVersion`            | `healthProbe()` returns `{ healthy: false }`                                       | 3 new Vitest tests                   |
| **macOS**       | `HealthResult` struct                    | `JSONSerialization` from response `Data` | `checkHealth()` returns `.healthy = false`                                         | 3 new Swift tests + `MockURLSession` |
| **Rust (gRPC)** | —                                        | —                                        | **Pending** — requires gRPC health check RPC or HTTP fallback (see gap table §7.7) | —                                    |

#### Backward compatibility

If the server does not include `protocolVersion` in the `/health` response, all clients treat the server as compatible (`healthy: true`, `protocolVersion: undefined`). This preserves compatibility with pre-negotiation server deployments.

#### Files changed

```
dashboard/src/lib/client.ts                          # healthProbe returns HealthResult, assess checks version
dashboard/src/lib/client.test.ts                     # +3 tests (compatible, incompatible, missing version)
vscode-extension/src/utils/api.ts                    # healthProbe returns { healthy, protocolVersion? }
vscode-extension/src/utils/api.test.ts               # +3 tests
macos/.../SkillsCore/Sources/SkillsCore/STXPublisher.swift
                                                     # checkHealth() returns HealthResult, parses JSON body
macos/.../SkillsCore/Tests/SkillsCoreTests/MockURLSession.swift
                                                     # new minimal URLSession mock for network tests
macos/.../SkillsCore/Tests/SkillsCoreTests/SkillsCoreTests.swift
                                                     # +3 capability negotiation tests
```

### 7.9 Keychain Token Storage — §6.1 (Completed 2026-05-30)

> Per CLIENT-SPEC.md §6.1: macOS credential model uses **macOS Keychain** as secondary source (primary: `SKILLPACK_TOKEN` env var).

#### Implementation

Created `KeychainTokenStore.swift` wrapping `Security.framework` (`SecItemAdd` / `SecItemCopyMatching` / `SecItemDelete`) with:
- **Service**: `io.ckodex.skillpack`
- **Account**: `skillpack-token`
- **Accessibility**: `kSecAttrAccessibleWhenUnlockedThisDeviceOnly`

`STXPublisher.init()` now resolves `bearerToken` with priority:
1. `SKILLPACK_TOKEN` environment variable
2. `KeychainTokenStore.load()` fallback

#### API

```swift
KeychainTokenStore.save(token: String) -> Bool
KeychainTokenStore.load() -> String?
KeychainTokenStore.delete() -> Bool
```

#### Tests

- `keychainRoundTripSaveLoadDelete()` — save → load → update → delete cycle
- `envVarTakesPriorityOverKeychain()` — documents the priority chain via `STXPublisher` internal init

#### Files changed

```
macos/.../SkillsCore/Sources/SkillsCore/KeychainTokenStore.swift  # new
macos/.../SkillsCore/Sources/SkillsCore/STXPublisher.swift        # env var > Keychain fallback
macos/.../SkillsCore/Tests/SkillsCoreTests/SkillsCoreTests.swift  # +2 Keychain tests
```

### 7.10 Server Infrastructure — TTL Cache, Event Bus, gRPC Streaming, SSE, REST Mutations (Completed 2026-05-30)

> Per CLIENT-SPEC.md §3.2 (gRPC streaming), §4.1 (cache invalidation), §4.2 (optimistic updates), §4.3 (event delivery).

#### TTL Cache (`crates/skillpack-api/src/cache.rs`)

- `CacheEntry<V>` — wraps values with `Instant` creation timestamp
- `TtlCache<K, V>` — HashMap with per-entry TTL, `get()` returns `None` if expired
- Methods: `insert`, `invalidate`, `invalidate_where`, `iter`, `values`
- Wired into `QueryServiceImpl` with `TTL_SKILL_LIST = 5 min` (others defined per §4.1)

#### Event Bus (`EventBus` in same file)

- `tokio::sync::broadcast` channel, capacity 256
- `ServerEvent` enum (now `Serialize`): `SkillCreated`, `SkillUpdated`, `SkillDeleted`, `AssessmentCompleted`, `SyncStarted`, `SyncCompleted`, `PreferenceChanged`
- `Arc<EventBus>` shared across gRPC services + HTTP server in `server.rs` binary
- `CanonicalStoreServiceImpl` publishes events on `sync_agents`, `import_skill`, `promote_skill`

#### gRPC Streaming

| Stream             | Before                  | After                                                                                                  |
| ------------------ | ----------------------- | ------------------------------------------------------------------------------------------------------ |
| `AssessStream`     | Placeholder dummy event | Real sequential checker loop: `DimensionProgress` → check → `DimensionComplete` → `AssessmentComplete` |
| `SyncAgentsStream` | Placeholder dummy event | Subscribes to `EventBus`, forwards `SyncStarted`/`SyncCompleted` as `SyncEvent` proto                  |

#### HTTP Server (`crates/skillpack-api/src/http_server.rs`)

Runs on separate port (`SKILLPACK_HTTP_PORT`, default 50052) alongside gRPC (50051).

| Endpoint        | Method | Description                                                              |
| --------------- | ------ | ------------------------------------------------------------------------ |
| `/health`       | GET    | JSON `{ status, protocol_version }` — capability negotiation per §3.1    |
| `/events`       | GET    | SSE `text/event-stream` — subscribes to `EventBus`, forwards JSON events |
| `/skills`       | POST   | Create skill in shared cache, publish `SkillCreated`                     |
| `/skills/{ref}` | PATCH  | Update skill in shared cache, publish `SkillUpdated`                     |
| `/skills/{ref}` | DELETE | Remove skill from cache, publish `SkillDeleted`                          |

#### QueryService Cache Invalidation

- `QueryServiceImpl::new` spawns a background `tokio::task` that subscribes to the event bus
- On `SkillCreated` / `SkillUpdated` / `SkillDeleted`: invalidates the matching cache entry
- On `SyncCompleted`: flushes the entire cache

#### Optimistic Update Loop (§4.2)

```
REST client → POST /skills → HTTP handler mutates shared cache
                                    ↓
                           EventBus::publish(SkillCreated)
                                    ↓
            ┌───────────────────────┼───────────────────────┐
            ↓                       ↓                       ↓
    SSE /events subscriber   QueryService cache task   gRPC SyncAgentsStream
            ↓                       ↓                       ↓
    push to browser         invalidate entry          stream to desktop client
```

#### Files changed

```
crates/skillpack-api/src/cache.rs                         # TtlCache + EventBus + ServerEvent + Serialize
crates/skillpack-api/src/auth.rs                          # new: TokenValidator + axum middleware + tonic interceptor
crates/skillpack-api/src/lib.rs                           # pub mod cache; pub mod http_server; pub mod auth
crates/skillpack-api/src/query_service.rs                 # TtlCache; event bus subscriber background task; tests
crates/skillpack-api/src/canonical_service.rs             # Arc<EventBus>; publish events; real SyncAgentsStream; RollbackSync; PackageSkill; PullSkill
 crates/skillpack-api/src/server.rs                        # real AssessStream with dimension progress
 crates/skillpack-api/src/http_server.rs                   # new: axum router with /health, /events, /skills + auth; registry hub endpoints
 crates/skillpack-api/src/bin/server.rs                    # spawn HTTP + gRPC concurrently; shared state; graceful shutdown; watcher task
 crates/skillpack-api/src/watcher.rs                       # new: notify-based filesystem watcher with debounced auto-sync
 crates/skillpack-api/src/cache.rs                         # added SyncProgress ServerEvent variant
 crates/skillpack-api/Cargo.toml                           # +axum, +serde, +tower (dev), +tower-http, +notify
 crates/skillpack-application/src/sync_canonical_store.rs  # IndexFile sync for Cursor (.mdc) and Windsurf (index.json)
 crates/skillpack-application/src/migrate_skills.rs        # shared Arc<IPGuard>
 crates/skillpack-domain/src/canonical_store.rs            # added namespace field; resolved_root() with namespace subdir
 crates/skillpack-adapters/src/persistence/duckdb.rs       # DuckDbRepository::open(path); canonical_skills, agent_sync_state, sync_history tables
 crates/skillpack-adapters/src/persistence/mod.rs          # re-export CanonicalSkillRecord, AgentSyncState
 crates/skillpack-adapters/src/grpc_client.rs              # SyncAgentsRequest namespace field
 proto/skillpack/v1/canonical.proto                        # +namespace, RollbackSync, PackageSkill, PullSkill RPCs and messages
 Cargo.toml                                                # +axum, +tower-http, +notify workspace dependency
 Dockerfile                                                # new: multi-stage build for skillpack-server + skillpack CLI
 docker-compose.yml                                        # new: compose with file-backed DuckDB volume + healthcheck
 ```

## §8. Codegraph Schema Navigation & Reorganization Plan **[C]**

> Systematic navigation of all schema-related symbols using `codegraph query` across the entire codebase. Goal: identify drift, duplication, and consolidation opportunities for the schema layer.

### 8.1 Methodology

```
codegraph query -p . -l 400 --json <symbol> > /tmp/<symbol>_results.json
```

Symbols queried: `client-model`, `Grade`, `DimensionScore`, `SkillSummary`, `validate`, `schema`, `health`, `token`, `auth`, `timeout`, `service`, `protocol`, `error`, `check`, `import`, `export`

### 8.2 Schema Source of Truth Hierarchy

```
schemas/skills-specs-next/client-model.schema.json (284 lines)
    ├── [P1] schemas/skills-specs-next/ckodex-skill-spec-v1.1/schemas/skill.v1.1.schema.json
    ├── [P1] schemas/cnsb/v1/cnsb.schema.json
    ├── [P1] schemas/common/types.schema.json
    ├── [P2] schemas/cnaab/v1/cnaab.schema.json
    ├── [P2] schemas/evidence/v1/envelope.schema.json
    └── [P2] schemas/prove/v1/policy.schema.json
```

**Finding:** `client-model.schema.json` is the ONLY schema consumed by all 4 client runtimes. All other schemas serve artifact validation (CNSB, CNAAB, evidence, policy) and are NOT part of the client transport contract.

### 8.3 Generated Binding Landscape

| Language   | Output Path                                               | Size       | CI Gate                  | Consumers (via codegraph)                                                                                                                                                                                          |
| ---------- | --------------------------------------------------------- | ---------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| TypeScript | `dashboard/src/generated/client-model.ts`                 | 339 lines  | `make check-schema-sync` | `dashboard/src/lib/client.ts`, `dashboard/src/lib/client.test.ts`, `dashboard/src/components/SkillScorecard.tsx`, `dashboard/src/components/RadarChart.tsx`                                                        |
| TypeScript | `vscode-extension/src/generated/client-model.ts`          | 339 lines  | `make check-schema-sync` | `vscode-extension/src/utils/api.ts`, `vscode-extension/src/utils/api.test.ts`, `vscode-extension/src/commands/registry.ts`, `vscode-extension/src/providers/registryTree.ts`, `vscode-extension/src/utils/html.ts` |
| Rust       | `crates/skillpack-adapters/src/generated/client_model.rs` | 461 lines  | `cargo check`            | `crates/skillpack-adapters/src/cli/mod.rs` (`ClientError` import), `crates/skillpack-adapters/src/grpc_client.rs` (types via canonical store)                                                                      |
| Swift      | `macos/.../SkillsCore/Generated/ClientModel.swift`        | ~299 lines | `swift build`            | `macos/.../SkillsCore/STXPublisher.swift`, `macos/.../SkillsCore/SkillsError.swift`                                                                                                                                |

### 8.4 Type Drift Detected by codegraph

#### Drift D-1: Duplicate `Grade` definitions (4 sources)

| Source               | Kind          | File                                                          | Status                                   |
| -------------------- | ------------- | ------------------------------------------------------------- | ---------------------------------------- |
| Canonical generated  | `enum`        | `crates/skillpack-adapters/src/generated/client_model.rs:244` | **[C]** Single source                    |
| Domain runtime       | `enum`        | `crates/skillpack-domain/src/score.rs:54`                     | **[C]** Separate domain concern — kept   |
| CLI command enum     | `enum_member` | `crates/skillpack-adapters/src/cli/mod.rs:276`                | **[C]** Command variant, not type — kept |
| SkillQA command enum | `enum_member` | `crates/skillpack-api/src/bin/skillqa.rs:39`                  | **[C]** Command variant, not type — kept |

**Verdict:** No action. Domain `Grade` (scoring logic) and canonical `Grade` (wire format) are semantically different layers.

#### Drift D-2: Legacy `skill.type.ts` still consumed

| Consumer                | Import target        | Fields used                                                |
| ----------------------- | -------------------- | ---------------------------------------------------------- |
| `schemas/skill.type.ts` | N/A (self-contained) | `Skill`, `Lifecycle`, `ASC`, `ResourceMetadata` interfaces |

**Status:** `skill.type.ts` predates `client-model.schema.json`. It defines the v1.1 skill manifest TypeScript types. No client runtime imports it today.

**Action:** Move to `schemas/skills-specs-next/ckodex-skill-spec-v1.1/types/` as historical reference; add deprecation header.

#### Drift D-3: Dashboard local adapter types

| Type            | File                                             | Canonical equivalent | Gap                                             |
| --------------- | ------------------------------------------------ | -------------------- | ----------------------------------------------- |
| `DimensionData` | `dashboard/src/components/DimensionChart.tsx:14` | `DimensionScore`     | Missing `findings: string[]` field in canonical |

**Action:** Add `findings?: string[]` to `client-model.schema.json` `$defs/DimensionScore`, then regenerate. Dashboard `DimensionData` can then alias canonical `DimensionScore`.

#### Drift D-4: Rust gRPC client uses domain types, not canonical

| File                                        | Type used                                         | Canonical equivalent                    |
| ------------------------------------------- | ------------------------------------------------- | --------------------------------------- |
| `crates/skillpack-api/src/server.rs`        | `GradeRequest`, `GradeResponse` (proto-generated) | `AssessmentResult` (canonical)          |
| `crates/skillpack-api/src/query_service.rs` | `Grade` (domain), `GradeSkillRequest/Response`    | `Grade`, `AssessmentResult` (canonical) |

**Verdict:** API server uses proto-generated types (internal gRPC contract). Canonical types are for CLIENT→SERVER wire format. These are different boundaries — no action unless we unify proto + canonical schema.

### 8.5 Reorganization Plan (3 Phases)

#### Phase 1: Consolidate Generation Pipeline (S — completed 2026-05-30)

| Step | Action                                                                | Owner                   | Status  | Evidence                                                                              |
| ---- | --------------------------------------------------------------------- | ----------------------- | ------- | ------------------------------------------------------------------------------------- |
| 1    | Move all generation scripts into `scripts/generate-types/`            | `scripts/`              | `[S]`   | `generate-ts-types.py`, `generate-rust-types.py`, `generate-swift-types.py`           |
| 2    | `make generate-types` already runs all generators; renamed in docs    | `Makefile`              | **[C]** | One command, one schema input (`SCHEMA` var)                                          |
| 3    | Add schema path + generation timestamp header to ALL generated files  | `Makefile` + `scripts/` | **[C]** | TS: `/* Auto-generated from $(SCHEMA) */` + timestamp; Rust/Swift: matching           |
| 4    | Deduplicate `client-model.ts` — generate once, copy to both consumers | `Makefile`              | **[C]** | `npx json2ts` once → `dashboard/.../client-model.ts` → `cp` to `vscode-extension/...` |

**Phase 1 implementation details:**
- `Makefile` now defines `GEN_TIMESTAMP := $(shell date -u +%Y-%m-%dT%H:%M:%SZ)` used by all generators.
- TypeScript types are generated **once** via `npx json2ts` (using dashboard's installed dependency), prepended with a 4-line header block, written to `dashboard/src/generated/client-model.ts`, then `cp` to `vscode-extension/src/generated/client-model.ts`.
- Rust generator script `scripts/generate-rust-types.py` accepts optional 3rd argument `[timestamp]` and injects it into `RUST_HEADER_TEMPLATE`.
- Swift `ClientModel.swift` header updated manually (Swift types are hand-maintained per existing `Makefile` note).
- `make check-schema-sync` strips the 4-line custom header before diffing, so drift detection is timestamp-agnostic.

#### Phase 2: Eliminate Non-Canonical Type Definitions (M — completed 2026-05-30)

| Target                              | Current location                              | Status  | Evidence                                                                                                                                                                                                                 |
| ----------------------------------- | --------------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `DimensionData` local interface     | `dashboard/src/components/DimensionChart.tsx` | **[C]** | Removed `DimensionData`; `DimensionChart` now accepts `DimensionScore[]` from canonical types. Mapping `dimensionName→dimension`, `maxScore→fullMark` moved inside component. `chartData` local removed from `page.tsx`. |
| `SkillSearchResult` local extension | `vscode-extension/src/utils/html.ts`          | **[C]** | Removed unused `SkillSearchResult` interface (was exported but never imported anywhere). `getSkillDetailsHtml` continues to use canonical `SkillSummary`.                                                                |
| `skill.type.ts` legacy definitions  | `schemas/skill.type.ts`                       | **[C]** | Moved to `schemas/_archive/skill.type.ts` with tombstone header. No runtime consumers affected.                                                                                                                          |
| Root `cnsb.schema.json`             | `schemas/cnsb.schema.json`                    | **[C]** | `$comment` deprecation already present (T-12 was previously completed).                                                                                                                                                  |

**Phase 2 notes:**
- `findings?: string[]` was already present in `client-model.schema.json` `$defs/DimensionScore` — no schema change required.
- Dashboard and vscode-extension both compile clean (`tsc --noEmit`).

#### Phase 3: CI Drift Detection (S — completed 2026-05-30)

| Check                                         | Implementation                                                                                  | Status  | Evidence                                                                            |
| --------------------------------------------- | ----------------------------------------------------------------------------------------------- | ------- | ----------------------------------------------------------------------------------- |
| Reject local type definitions                 | `scripts/check-drift.py` scans for `interface.*Score` / `interface.*Skill` outside `generated/` | **[C]** | Script created at `scripts/check-drift.py`; `make check-schema-drift` target added. |
| Verify all consumers import from `generated/` | Already covered by `check-schema-sync` (diffs committed files against fresh generation)         | **[C]** | `make check-schema-sync` verifies TS + Rust generated files match schema.           |
| Protobuf ↔ canonical alignment                | Diff proto `Grade` against canonical `Grade`; flag on mismatch                                  | `[S]`   | Future — requires proto schema generation from JSON schema.                         |

### 8.6 Cross-Reference to Existing TODOs

| New finding                     | Maps to existing TODO    | Status                        |
| ------------------------------- | ------------------------ | ----------------------------- |
| `skill.type.ts` archive         | T-12 (retire root cnsb)  | Related — same cleanup sprint |
| `DimensionData` → canonical     | T-15 (synopsisHash/tier) | Independent — can parallelize |
| Generation script consolidation | None — new task          | Proposed as T-27..T-29        |

### 8.7 Files Requiring No Action (Verified by codegraph)

These files use canonical types correctly and have no drift:

- `dashboard/src/lib/client.ts` — re-exports canonical types ✓
- `vscode-extension/src/utils/api.ts` — imports canonical `ClientError`, `SkillSummary`, `SkillDetail`, `RegistryEntry` ✓
- `crates/skillpack-adapters/src/cli/mod.rs` — imports canonical `ClientError` via `crate::generated::client_model` ✓
- `macos/.../SkillsCore/SkillsError.swift` — wraps canonical `ClientError` in `SkillsError.clientError()` case ✓

---

*End of Dossier v0.2 (amended — §7 CLIENT-SPEC compliance + §7.8 capability negotiation + §7.9 Keychain storage + §7.10 server infrastructure + §8 Codegraph navigation + §11 AIPACK alignment)*
